...
def preprocess(data_root='../datasets/dexycb', split='s0', mode='test', side='right'):
    ...
    with open(f'{data_root}/dexycb_{mode}_{split}.json', 'r') as f:
        dexycb_data = json.load(f)
    data_ids = [dexycb_data['images'][i]['file_name'] for i in range(len(dexycb_data['images']))]

    with open(f'{data_root}/dexycb_{mode}_{split}_t.json', 'w') as json_data:
        coco_file = dict()
        data_images = []
        data_annos = []
        ...
        for i in tqdm(range(len(dataset))):
            ...
            if sample['mano_side'] in side:
                ...
                if (img_info['file_name'] in data_ids and mode == 'train') or mode == 'test':
