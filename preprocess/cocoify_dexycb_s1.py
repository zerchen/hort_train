...
def preprocess(data_root='../datasets/dexycb', split='s0', mode='train'):
    ...
    mano_layer_right = ManoLayer(flat_hand_mean=False, ncomps=45, side='right', mano_root='../common/mano/assets/', use_pca=True).cuda()
    mano_layer_left = ManoLayer(flat_hand_mean=False, ncomps=45, side='left', mano_root='../common/mano/assets/', use_pca=True).cuda()

    with open(f'{data_root}/dexycb_{mode}_s1_t.json', 'w') as json_data:
        coco_file = dict()
        data_images = []
        data_annos = []
        ...
        for i in tqdm(range(len(dataset))):
            ...
            subject = int(img_path.split('/data/')[-1].split('/')[0].split('-')[-1])
            video_id = img_path.split('/data/')[-1].split('/')[1]
            sub_video_id = img_path.split('/data/')[-1].split('/')[2]
            frame_idx = int(img_path.split('/data/')[-1].split('/')[-1].split('_')[-1].split('.')[0])

            if frame_idx % 5 != 0 and mode == "test":
                continue

            label = np.load(sample['label_file'])
            hand_joints_2d = label['joint_2d'][0]
            if np.all(hand_joints_2d) == -1.0 or np.any(hand_joints_2d <= 0) or np.any(hand_joints_2d[:, 0] >= 640) or np.any(hand_joints_2d[:, 1] >= 480):
                continue

            ho_seg = label['seg']
            grasp_obj_id = sample['ycb_ids'][sample['ycb_grasp_ind']]
            hand_seg = (ho_seg == 255)
            obj_seg = (ho_seg == grasp_obj_id)
            connected = connected_mask(hand_seg, obj_seg)
            if not connected:
                continue

            img_info['id'] = sample_id
            sample_name = '_'.join([str(subject), video_id, sub_video_id, str(frame_idx)])
            ...
            if sample['mano_side'] == "right":
                hand_verts, _, hand_poses, _, _ = mano_layer_right(torch.from_numpy(label['pose_m'][:, 0:48]).cuda(), betas.cuda(), torch.from_numpy(label['pose_m'][:, 48:51]).cuda())
            elif sample['mano_side'] == "left":
                hand_verts, _, hand_poses, _, _ = mano_layer_left(torch.from_numpy(label['pose_m'][:, 0:48]).cuda(), betas.cuda(), torch.from_numpy(label['pose_m'][:, 48:51]).cuda())
            hand_verts = hand_verts[0].cpu().numpy().tolist()
            np.save(os.path.join(output_verts_3d_dir, f"{sample_name}.npy"), hand_verts)
            ...
            anno_info['hand_poses'] = hand_poses[0].cpu().numpy().tolist()
            anno_info['hand_trans'] = label['pose_m'][0, 48:].tolist()
            anno_info['hand_shapes'] = sample['mano_betas']
            anno_info['hand_side'] = int(sample['mano_side'] == "right")
            ...
            obj_affine_transform = np.concatenate([np.concatenate([R, t], axis=1), np.array([[0.0, 0.0, 0.0, 1.0]], dtype=np.float32)])
            obj_corners = (R @ obj_rest_corners.transpose(1, 0) + t).transpose(1, 0)

            obj_rest_verts = obj_rest_mesh.vertices
            obj_verts = (R @ obj_rest_verts.transpose(1, 0) + t).transpose(1, 0)

            # intr_mat = np.zeros((3, 4), dtype=np.float32)
            # intr_mat[0, 0] = anno_info['fx']
            # intr_mat[0, 2] = 640 - anno_info['cx']
            ...
            # img = cv2.flip(img, 1)
            # homo_obj_verts = np.ones((hand_verts.shape[0], 4), dtype=np.float32)
            # homo_obj_verts[:, :3] = hand_verts
            ...
            #     if verts_2d[i, 0] >= 0 and verts_2d[i, 0] <= 640 and verts_2d[i, 1] >= 0 and verts_2d[i, 1] <= 480:
            ...
            anno_info['obj_center_3d'] = t.squeeze().tolist()
        ...
        with open(f'{data_root}/splits/s1_{mode}.json', 'w') as f:
