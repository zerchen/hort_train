...
def main():
    ...
    if args.slurm:
        ...
        os.environ['MASTER_PORT'] = str(29100)
    ...
    if args.slurm:
        init_signal_handler()
    # train
    for _ in range(trainer.start_epoch):
        ...
        for itr, (inputs, targets, metas) in enumerate(trainer.batch_generator):
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):
                if cfg.task in ['hsdf_osdf_1net', 'hsdf_osdf_2net', 'hsdf_osdf_2net_pa', 'hsdf_osdf_2net_video', 'hsdf_osdf_2net_video_pa', 'pose_sdf', 'hsdf_osdf_1net_plus', 'hsdf_osdf_1net_plus_pa', 'hsdf_osdf_3net_pa', 'osdf_video', 'osdf_1net']:
                    ...
                    loss, pc_results = trainer.model(inputs, targets, metas, 'train')
                all_loss = sum(loss[k] for k in loss)

            # backward
            trainer.scaler.scale(all_loss).backward()
